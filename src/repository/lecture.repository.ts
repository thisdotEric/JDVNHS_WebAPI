...
class LectureRepository {
  ...
  async getAllLectures(subject_id: string) {
    const lectures = await this.db
      .getDbInstance()
      .raw(
        `select l.lecture_id, l.lecture_date, l.subject_id, l.grading_period, l.code, lc."learning_competency" from lectures l join learning_competency lc on l."code" = lc."code" where l.subject_id = '${subject_id}'`
      );
